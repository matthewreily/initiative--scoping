...
- **Secrets**: the DB connection string is generated by Terraform; the Entra client secret is created empty and populated by hand so it never enters Terraform state or the repo.
...
2. Register the app in Entra ID (web platform, redirect URI `https://<cloud-run-url>/signin-oidc`, app roles `Administrator`, `InitiativeOwner`, `Contributor`, `Viewer`, `FinancePmo`), and create a client secret.
...
8. Add the Cloud Run URL (`terraform output service_url`) as a redirect URI on the Entra app registration.
