...
1. `Initiatives → New`: name, business unit, planning mode, target start (and target end for fixed duration), sizing method (`Direct` hours or a relative size). The size dropdown only offers sizes that have an allocation template for the chosen method; add templates under `Admin → Sizing` first.
