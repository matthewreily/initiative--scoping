...
3. Add **allocations**: phase, resource type, seniority, location, class (Internal FTE / Vendor), quantity and hours (effort-driven) or staffing % (fixed duration). For relative sizing choose a size and *Apply size* to generate allocations from the template, then adjust. In fixed-duration mode *Apply size* splits the target window across the template's phases in proportion to their share of hours and derives the staffing % that yields the size's hours – e.g. “L in 12 weeks needs 1 FTE”. To switch an existing effort-driven initiative to fixed duration, set a target end; existing hours are converted to an equivalent staffing % per phase.
