...
/// </summary>
public class GraphDirectorySearch(HttpClient http, IConfiguration config, ILogger<GraphDirectorySearch> logger) : IDirectorySearch
{
    ...
    public async Task<DirectorySearchResult> SearchAsync(string term, CancellationToken ct)
    {
        ...
        try
        {
            var accessToken = await GetTokenAsync(ct);
