...
- **Allocations** – phase × resource type × seniority × location × internal/vendor × quantity × estimated hours (per person; forecast = quantity × hours × rate). Seniority lives on the allocation, not the resource type. Each allocation (and non-labor line) carries a `CapitalizationType` (Opex default / Capex) that `MonthlyPhasing.ByFiscalPeriod` uses to split fiscal-year and quarter subtotals into Capex / Opex; baseline lines snapshot it and scenarios copy it. An allocation may optionally name a roster **person** (`PersonId`) whose resource type / seniority / BU / class / vendor match it (quantity must be 1); pricing is unchanged, baseline lines snapshot `PersonName`, Capacity gains a per-person view and actuals imports match by name against people named on the initiative.
