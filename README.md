...
- **Seniority** – admin-managed catalog (seeded with Associate/Mid/Senior/Staff/Principal; sort order + active flag). Rate-card and People CSV imports add unknown seniority names to the catalog automatically, so vendor ladders such as `Level 1 (0-2 Years)` import as-is.
- **Resourcing classes** – admin-managed catalog replacing the old fixed `InternalFte`/`Vendor` enum (seeded *Internal* = 1 and *Vendor* = 2 so existing data migrates in place). Each class carries `IsVendor` (drives the Vendor picker, rate matching and vendor rollups), a `DefaultCapexPercent` for new labor lines, sort order and active flag. CSV import/export use the class name; `InternalFte` is still accepted on import.
- **Rate Cards** – Draft → Published → Retired lifecycle with an effective start date. Entries are global (shared by every business unit) and keyed by resource type × seniority × location × internal/vendor (× vendor). Entries can be added inline or bulk-imported via CSV (`ResourceType,Seniority,Location,ResourcingClass,HourlyRate[,Vendor][,Discipline]`; a legacy `BusinessUnit` column is ignored; unknown resource types and seniority names are added to their catalogs – a new resource type takes the optional `Discipline` or an *Unassigned* discipline; merge or replace; the file is rejected as a whole if any row is invalid). Export and a template download are available. Published cards cannot be deleted – retire them so historical baselines stay reproducible.
...
- **Work calendar** – hours per working day (single row, default 8), the fiscal-year start month (1–12, default 1 = calendar year; drives `FiscalCalendar` labels such as `FY2027` / `FY2027 Q1`) and a holiday list (unique weekday dates). Working days are Mon–Fri minus holidays; used by fixed-duration initiatives.
- **Cost catalog** – `CostCatalogItem` rows (non-labor `CostCategory` × name, unique per category; vendor, `BillingModel` One-time/Monthly/Annual, unit cost, default `CapexPercent` 0–100, active flag) that initiatives can pick to prefill non-labor lines. `Labor` is rejected as a category (labor is priced from rate cards); referenced items cannot be deleted.
...
- **Allocations** – phase × resource type × seniority × location × internal/vendor × quantity × estimated hours (per person; forecast = quantity × hours × rate). Seniority lives on the allocation, not the resource type. Each allocation (and non-labor line) carries a `CapexPercent` (0–100, default 0 = all Opex, 100 = all Capex); `CapexSplit` apportions a line's cost as Capex = round(cost × % / 100, 2) and Opex = cost − Capex, which `MonthlyPhasing.ByFiscalPeriod` uses for the fiscal-year and quarter Capex / Opex columns; baseline lines snapshot the percentage and scenarios copy it. An allocation may optionally name roster **people** (`InitiativeAllocationPerson` rows, at most `Quantity`) whose resource type / seniority / BU / class / vendor match it; each fills one seat and the rest are `UnassignedSeats`. Pricing is unchanged, baseline lines snapshot one line per named seat (`PersonId`/`PersonName`) plus one for the unnamed seats, Capacity gains a per-person view and actuals imports match by name against people named on the initiative.
...
- **Non-labor costs** – `InitiativeNonLaborCost` lines (category, optional catalog item, description, billing model, quantity, unit cost, optional phase scope and explicit start/end, contract reference, cost centre). `NonLaborCostCalculator` bills whole periods counted from the window start (a partial month/year counts as full; one-time bills once) over the explicit dates, else the phase window, else the initiative's `TargetStart`–`TargetEnd`. `ForecastResult` exposes `LaborCost`, `NonLaborCost` and `TotalCost` (`TotalHours` stays labor-only); baselines freeze `ForecastBaselineNonLaborLine` rows; variance adds a by-category view; `ActualAdjustment.Category` lets invoices land against the right category; portfolio and exports carry a non-labor column. An empty billing window blocks activation.
...
- **Activate** requires at least one phase, at least one allocation and no unpriced lines. It freezes the live forecast as **Forecast Baseline v1** (`ForecastBaseline` + per-line `HourlyRate`/`Cost`), sets the status to **Active** and locks scope. Baseline values never change when rate cards are later republished. Administrators activate directly; an Owner member instead **requests activation** (optional note, e-mail to administrators), and an Administrator **approves** – which activates and captures v1 in one step – or **rejects** from the initiative page or the `/Approvals` queue (e-mail back to the requester). Requests can be withdrawn while pending and are withdrawn automatically when the initiative is cancelled.
- **Status transitions**: Draft → Active (via Activate) / Cancelled; Active → OnHold / Complete / Cancelled; OnHold → Active / Cancelled. Complete is terminal. A Cancelled initiative can be **reopened** (back to Draft if it was never baselined, otherwise On hold with its baselines intact) or **deleted**; Draft and Cancelled initiatives can be deleted, except a Cancelled one with recorded actuals, which stays for the financial record.
- **Re-baseline** (spec §5.4): an Owner requests a re-baseline with a reason → an **Administrator** approves (or rejects) from the initiative page or the `/Approvals` queue → scope is unlocked → the Owner **finalizes**, which captures baseline v*N+1*, marks it current and locks scope again. Every prior version is retained; `/Initiatives/{id}/Baselines` lists versions, line-level deltas vs. the previous version and the live-forecast drift vs. the current baseline. Withdrawing an open request re-locks scope; Cancelling an initiative withdraws any open request.
- **Change requests** – a formal per-initiative change log (`ChangeRequest`, codes CR-1, CR-2, … per initiative) for scope / schedule / cost / resource changes on Active initiatives: title, description, reason, requester, optional estimated cost / hours impact and proposed target end, plus a *before* snapshot of the live forecast, current baseline version and target end. Administrators approve or reject (with a note) from the **Changes** tab or the `/Approvals` queue; approving on an Active initiative opens (or joins) an approved re-baseline, and finalizing that re-baseline marks the linked requests **Implemented** with the *after* forecast, realised impact and resulting baseline version. Portfolio flags pending / approved changes and both exports carry a **Change requests** table; every step is audited (`ChangeRequest`, `ChangeRequestDecision`, `ChangeRequestImplemented`).
...
- **Import** (`/Actuals`, Administrator or Finance/PMO) – upload a CSV `ExternalProjectId,ExternalPersonId,WorkDate,Hours[,Cost][,Reference]` (`WorkDate` = `yyyy-MM-dd`; template at `/Actuals/Template`). Files with any invalid row are rejected before anything is written. Rows are matched to an initiative via source mappings and to a person via external IDs (or, failing that, the display name of exactly one person named on that initiative's allocations); a row missing either lands in the **unmapped queue**. `Cost` from the source wins; otherwise cost = hours × the exact published rate in effect on the work date (no fallback — a missing rate leaves the row *unpriced*, counted as $0 and flagged). Rows whose `(Source, Reference)` was already imported are skipped, so re-uploading a file is idempotent. Every import records who/when/file/counts/log; `/Actuals/Imports/{id}` lists its rows.
...
See [`deploy/gcp/README.md`](deploy/gcp/README.md): Terraform for Cloud Run + Cloud SQL (PostgreSQL 16) + Secret Manager + Artifact Registry + Workload Identity Federation, and `.github/workflows/deploy.yml` which builds the image, runs the migrate job, deploys and smoke-tests `/health`. Live URLs, console links and identifiers for every environment are listed in [`ENVIRONMENTS.md`](ENVIRONMENTS.md). An interactive architecture diagram (hosting, data, identity, pipelines, OpenTelemetry) is in [`docs/environment-diagram.html`](docs/environment-diagram.html) — open it in a browser.
