...
- **Cost catalog** – `CostCatalogItem` rows (non-labor `CostCategory` × name, unique per category; vendor, `BillingModel` One-time/Monthly/Annual, unit cost, active flag) that initiatives can pick to prefill non-labor lines. `Labor` is rejected as a category (labor is priced from rate cards); referenced items cannot be deleted.
